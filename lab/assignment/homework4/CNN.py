...
class Conv2d(Layer):
    ...
    def compute_output_dims(h_in, w_in, h_f, w_f, h_p, w_p, stride):
        ...
        ##------Answer 1 begins-------- ##
        #---Attention: h_out and w_out must be integer
        h_out= int((h_in-h_f+2*h_p)/stride+1)
        w_out= int((w_in-w_f+2*w_p)/stride+1)
        ##--------Answer 1----- ends##
    ...
    def backward(self, da):
        ...
        for i in range(h_out):
            ...
            for j in range(w_out):
                ...
                # complete your code here
                # out[:, :, h_start:h_end, w_start:w_end] += da[:,:,np.newaxis,i:i+1,j:j+1]*self.w
...
class MaxPool2d(Layer):
    ...
    def compute_output_dims(h_in, w_in, h_f, w_f, stride):
        ...
        ##------Answer 3 begins-------- ##
        #---Attention: h_out and w_out must be integer
        h_out= int((h_in-h_f)/stride+1)
        w_out= int((w_in-w_f)/stride+1)
        ##--------Answer 3----- ends##
...
class FullyConnected(Layer):
    ...
    def forward(self, x):
        ...
        out=np.zeros((batch_size,self.w.shape[0]))
        for i in range(batch_size):
            for j in range(self.w.shape[0]):
                out[i,j]=sum(x[i,:]*self.w[j,:])+self.b[j]
    ...
    def backward(self, da):
        ...
        batch_size,out_features=da.shape

        self.db=np.sum(da, axis=0, keepdims=False)/batch_size
...
def main():
    ...
    model = SequentialModel([
        ...
        # complete your code here
        FullyConnected(in_features=int(((h-2)/2+1)*((w-2)/2+1)*3),out_features=int(num_classes)),
        Softmax()
    ])
